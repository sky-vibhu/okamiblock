...
type ContractCallArgs struct {
	...
	Nonce *big.Int
	// Amount is the aOKM amount to send in the transaction.
	Amount *big.Int
...
func DeployContract(
	...
	res, err := DeliverEthTx(okamiapp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, okamiapp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := okamiapp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := okamiapp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := okamiapp.EvmKeeper.GetNonce(ctx, from)
