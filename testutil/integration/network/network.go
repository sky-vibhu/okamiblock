...
func (n *IntegrationNetwork) configureAndInitChain() error {
	...
	// Create a new OkamiApp with the following params
	okamiapp := createOkamiApp(n.cfg.chainID)

	// Configure Genesis state
	genesisState := app.NewDefaultGenesisState()

	genesisState = setAuthGenesisState(okamiapp, genesisState, genAccounts)
	...
	genesisState = setStakingGenesisState(okamiapp, genesisState, stakingParams)
	...
	genesisState = setBankGenesisState(okamiapp, genesisState, bankParams)
	...
	header := tmproto.Header{
		ChainID:            n.cfg.chainID,
		Height:             okamiapp.LastBlockHeight() + 1,
		AppHash:            okamiapp.LastCommitID().Hash,
		ValidatorsHash:     valSet.Hash(),
		NextValidatorsHash: valSet.Hash(),
		ProposerAddress:    valSet.Proposer.Address,
	}
	okamiapp.BeginBlock(abcitypes.RequestBeginBlock{Header: header})

	// Set networks global parameters
	n.app = okamiapp
	// TODO - this might not be the best way to initilize the context
	n.ctx = okamiapp.BaseApp.NewContext(false, header)
