...
import (
	...
	"github.com/okamiblock/okamiblock/testutil/tx"
)
...
//  3. EndBlock
//  4. Commit
func Commit(ctx sdk.Context, app *app.Okami, t time.Duration, vs *tmtypes.ValidatorSet) (sdk.Context, error) {
...
func CommitAndCreateNewCtx(ctx sdk.Context, app *app.Okami, t time.Duration, vs *tmtypes.ValidatorSet) (sdk.Context, error) {
	header, err := commit(ctx, app, t, vs)
	if err != nil {
		return ctx, err
	}
...
func DeliverTx(
	...
	return BroadcastTxBytes(appOkami, txConfig.TxEncoder(), tx)
}
...
func DeliverEthTx(
	...
	tx, err := tx.PrepareEthTx(txConfig, appOkami, priv, msgs...)
	if err != nil {
		return abci.ResponseDeliverTx{}, err
	}
	res, err := BroadcastTxBytes(appOkami, txConfig.TxEncoder(), tx)
	if err != nil {
		return abci.ResponseDeliverTx{}, err
	}
...
func CheckTx(
	...
	return checkTxBytes(appOkami, txConfig.TxEncoder(), tx)
}
...
func BroadcastTxBytes(app *app.Okami, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ResponseDeliverTx, error) {
...
func commit(ctx sdk.Context, app *app.Okami, t time.Duration, vs *tmtypes.ValidatorSet) (tmproto.Header, error) {
...
func checkTxBytes(app *app.Okami, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ResponseCheckTx, error) {
	bz, err := txEncoder(tx)
	if err != nil {
		return abci.ResponseCheckTx{}, err
	}
