...
func PrepareEthTx(
	...
	signer := ethtypes.LatestSignerForChainID(appOkami.EvmKeeper.ChainID())
	txFee := sdk.Coins{}
	txGasLimit := uint64(0)
...
func CreateEthTx(
	...
	chainID := appOkami.EvmKeeper.ChainID()

	// When we send multiple Ethereum Tx's in one Cosmos Tx, we need to increment the nonce for each one.
	nonce := appOkami.EvmKeeper.GetNonce(ctx, fromAddr) + uint64(nonceIncrement)
	evmTxParams := &evmtypes.EvmTxArgs{
		...
		Amount:    amount,
		GasLimit:  100000,
		GasFeeCap: appOkami.FeeMarketKeeper.GetBaseFee(ctx),
