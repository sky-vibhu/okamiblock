...
var (
	...
	DefaultFee = sdk.NewCoin(utils.BaseDenom, sdk.NewIntFromUint64(uint64(feeAmt))) // 0.01 okami
)
...
type CosmosTxArgs struct {
	...
	// ChainID is the chain's id on cosmos format, e.g. 'okami_9000-1'
	ChainID string
...
func signCosmosTx(
	...
	seq, err := appOkami.AccountKeeper.GetSequence(ctx, addr)
	if err != nil {
		return nil, err
	}
	...
	accNumber := appOkami.AccountKeeper.GetAccount(ctx, addr).GetAccountNumber()
