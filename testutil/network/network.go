...
func DefaultConfig() Config {
	...
	chainID := fmt.Sprintf("okami_%d-1", tmrand.Int63n(9999999999999)+1)
	return Config{
		Codec:             encCfg.Codec,
		...
		NumValidators:     4,
		BondDenom:         "aOKM",
		MinGasPrices:      fmt.Sprintf("0.000006%s", okamitypes.AttoOkami),
		AccountTokens:     sdk.TokensFromConsensusPower(1000000000000000000, okamitypes.PowerReduction),
		StakingTokens:     sdk.TokensFromConsensusPower(500000000000000000, okamitypes.PowerReduction),
		BondedTokens:      sdk.TokensFromConsensusPower(100000000000000000, okamitypes.PowerReduction),
...
// NewAppConstructor returns a new Okami AppConstructor
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	if !okamitypes.IsValidChainID(cfg.ChainID) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "okamid")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "okamicli")
		...
		customAppTemplate, _ := config.AppConfig(okamitypes.AttoOkami)
