# Okami Contributor Guidelines

<!-- markdown-link-check-disable -->
- [Okami Contributor Guidelines](#okami-contributor-guidelines)
...
Thank you for considering making contributions to Okami and related repositories!

Okami uses [Tendermint’s coding repo](https://github.com/tendermint/coding)
...
1. Either [open](https://github.com/okami/okami/issues/new/choose)
   or [find](https://github.com/okami/okami/issues) an issue you have identified and would like to contribute to
   resolving.
2. Participate in thoughtful discussion on that issue.
3. If you would like to contribute:
    1. If the issue is a proposal, ensure that the proposal has been accepted by the Okami team.
    ...
    4. Follow standard Github best practices:
        1. Fork the repo
        2. Branch from the HEAD of `development`(For core developers working within the okami repo, to ensure a
    ...
    8. Please make sure to run `make format` before every commit -
       ...
       There are CI tests built into the Okami repository
...
Check out our [good first issues](https://github.com/okami/okami/issues?q=label%3A%22good+first+issue%22).

## <span id="adr">Architecture Decision Records (ADR)</span>

When proposing an architecture decision for Okami,
...
While my fork lives at `https://github.com/okami/okami`,
the code should never exist at `$GOPATH/src/github.com/okami/okami`.
Instead, we use `git remote` to add the fork as a new remote for the original repo,`$GOPATH/src/github.com/okami/okami`,
...
2. Go to the original repo checked out locally. (i.e. `$GOPATH/src/github.com/okami/okami`)
3. `git remote rename origin upstream`
4. `git remote add origin git@github.com:tharsis/okami.git`

Now `origin` refers to my fork and `upstream` refers to the okami version.
So I can `git push -u origin master` to update my fork,
and make pull requests to okami from there.
...
with [gogoproto](https://github.com/cosmos/protobuf) to generate code for use in Okami.
...
4. Create your feature branch from `development` either on `github.com/okami/okami`, or your fork (
   using `git remote add origin`).
5. Before submitting a pull request, begin `git rebase` on top of `development`.

## <span id="testing">Testing</span>

Okami uses [GitHub Actions](https://github.com/features/actions) for automated testing.

## <span id="updating_doc">Updating Documentation</span>

If you open a PR on the Okami repo, it is mandatory to update the relevant documentation in `/docs`. Please refer to
...
Okami utilizes [semantic versioning](https://semver.org/).
...
**Note**: tharsis’s Okami team currently cuts releases on a need to have basis.
