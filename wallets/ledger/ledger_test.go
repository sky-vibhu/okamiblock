...
func (suite *LedgerTestSuite) TestOkamiLedgerDerivation() {
	testCases := []struct {
		name     string
		mockFunc func()
		expPass  bool
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			derivationFunc := ledger.OkamiLedgerDerivation()
...
func (suite *LedgerTestSuite) TestGetAddressPubKeySECP256K1() {
	...
	addr := crypto.PubkeyToAddress(privKey.PublicKey)
	expAddr, err := sdk.Bech32ifyAddressBytes("okami", common.HexToAddress(addr.String()).Bytes())
	suite.Require().NoError(err)
