...
const DefaultFeeAmt = int64(150_000_000_000_000_000) // 0.15 OKAMI

var globalStartTime = time.Date(2020, 1, 2, 0, 0, 0, 0, time.UTC)

// NewCoordinator initializes Coordinator with N EVM TestChain's (Okami apps) and M Cosmos chains (Simulation Apps)
...
func SendMsgs(chain *ibctesting.TestChain, feeAmt int64, msgs ...sdk.Msg) (*sdk.Result, error) {
	...
	if okamiChain, ok := chain.App.(*app.Okami); ok {
		bondDenom = okamiChain.StakingKeeper.BondDenom(chain.GetContext())
	} else {
