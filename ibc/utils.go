...
func GetTransferSenderRecipient(packet channeltypes.Packet) (
	...
	// and change the bech32 human readable prefix (HRP) of the sender to `okami`
	sender, err = utils.GetOkamiAddressFromBech32(data.Sender)
	if err != nil {
		return nil, nil, "", "", errorsmod.Wrap(err, "invalid sender")
	}

	// validate the recipient bech32 address from the counterparty chain
	// and change the bech32 human readable prefix (HRP) of the recipient to `okami`
	recipient, err = utils.GetOkamiAddressFromBech32(data.Receiver)
	if err != nil {
