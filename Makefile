...
HTTPS_GIT := https://github.com/okami/okami.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=okami \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(OKAMI_BINARY) \
...
build-docker:
	...
	mkdir -p ./build/.okamid
	echo '#!/usr/bin/env bash' > ./build/okamid
	echo "IMAGE_NAME=${DOCKER_IMAGE}:${COMMIT_HASH}" >> ./build/okamid
	echo 'SCRIPT_PATH=$$(cd $$(dirname $$0) && pwd -P)' >> ./build/okamid
	echo 'docker run -it --rm -v $${SCRIPT_PATH}/.okamid:/home/okami/.okamid $$IMAGE_NAME okamid "$$@"' >> ./build/okamid
	chmod +x ./build/okamid
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/okami/okami"
	godoc -http=:6060
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(OKAMI_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/okami:Z okami/node "./okamid testnet init-files --v 4 -o /okami --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\okamid:/okami\Z okami/node "./okamid tendermint unsafe-reset-all --home=/okami"
	@docker run --rm -v $(CURDIR)\build\node1\okamid:/okami\Z okami/node "./okamid tendermint unsafe-reset-all --home=/okami"
	@docker run --rm -v $(CURDIR)\build\node2\okamid:/okami\Z okami/node "./okamid tendermint unsafe-reset-all --home=/okami"
	@docker run --rm -v $(CURDIR)\build\node3\okamid:/okami\Z okami/node "./okamid tendermint unsafe-reset-all --home=/okami"
else
	@docker run --rm -v $(CURDIR)/build/node0/okamid:/okami:Z okami/node "./okamid tendermint unsafe-reset-all --home=/okami"
	@docker run --rm -v $(CURDIR)/build/node1/okamid:/okami:Z okami/node "./okamid tendermint unsafe-reset-all --home=/okami"
	@docker run --rm -v $(CURDIR)/build/node2/okamid:/okami:Z okami/node "./okamid tendermint unsafe-reset-all --home=/okami"
	@docker run --rm -v $(CURDIR)/build/node3/okamid:/okami:Z okami/node "./okamid tendermint unsafe-reset-all --home=/okami"
endif
...
PACKAGE_NAME:=github.com/okami/okami
