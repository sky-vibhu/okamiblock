...
const (
	// MainnetChainID defines the Okami EIP155 chain ID for mainnet
	MainnetChainID = "okami_9001"
	// TestnetChainID defines the Okami EIP155 chain ID for testnet
	TestnetChainID = "okami_9000"
	// BaseDenom defines the Okami mainnet denomination
	BaseDenom = "aOKM"
)

// IsMainnet returns true if the chain-id has the Okami mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the Okami testnet EIP155 chain prefix.
...
// GetOkamiAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `okami`).
// The function fails if the provided bech32 address is invalid.
func GetOkamiAddressFromBech32(address string) (sdk.AccAddress, error) {
