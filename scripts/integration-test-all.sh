...
KEY="dev0"
CHAINID="okami_9000-1"
MONIKER="mymoniker"

## default port prefixes for okamid
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
DATA_DIR=$(mktemp -d -t okami-datadir.XXXXX)
...
init_func() {
    "$PWD"/build/okamid keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/okamid init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/okamid add-genesis-account \
    "$("$PWD"/build/okamid keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000aOKM,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/okamid gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/okamid collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/okamid validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    echo "starting okami node $i in background ..."
    "$PWD"/build/okamid start --pruning=nothing --rpc.unsafe \
    ...
    OKAMI_PID=$!
    echo "started okami node, pid=$OKAMI_PID"
    # add PID to array
    arr+=("$OKAMI_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test okami node $HOST_RPC ..."
...
stop_func() {
    OKAMI_PID=$i
    echo "shutting down node, pid=$OKAMI_PID ..."
