...
import (
	...
	"github.com/okamiblock/okamiblock/crypto/hd"
	...
	"github.com/okamiblock/okamiblock/testutil/network"
...
Example:
	okamid testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "okamid", "Home directory of the node's daemon configuration")
...
Example:
	okamid testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("okami_%d-1", tmrand.Int63n(9999999999999)+1)
	}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		kb, err := keyring.New(sdk.KeyringServiceName(), args.keyringBackend, nodeDir, inBuf, clientCtx.Codec, okamikr.Option())
		if err != nil {
			return err
		}
		...
		accStakingTokens := sdk.TokensFromConsensusPower(5000, okamitypes.PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(cmdcfg.BaseDenom, accStakingTokens),
		}

		genBalances = append(genBalances, banktypes.Balance{Address: addr.String(), Coins: coins.Sort()})
		genAccounts = append(genAccounts, &okamitypes.EthAccount{
			BaseAccount: authtypes.NewBaseAccount(addr, nil, 0, 0),
			CodeHash:    common.BytesToHash(evmtypes.EmptyCodeHash).Hex(),
		})

		valTokens := sdk.TokensFromConsensusPower(100, okamitypes.PowerReduction)
