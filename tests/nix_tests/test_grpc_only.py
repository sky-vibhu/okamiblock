...
from .network import Okami, create_snapshots_dir, setup_custom_okami
from .utils import (
    CONTRACTS,
    decode_bech32,
    deploy_contract,
...
def grpc_eth_call(port: int, args: dict, chain_id=None, proposer_address=None):
    ...
    return requests.get(f"http://localhost:{port}/okami/evm/v1/eth_call", params).json()


def test_grpc_mode(okami_cluster: Okami):
    """
    ...
    api_port = ports.api_port(okami_cluster.base_port(1))
    ...
    for i in range(2):
        wait_for_block(okami_cluster.cosmos_cli(i), 1)
    supervisorctl(okami_cluster.base_dir / "../tasks.ini", "stop", "okami_9000-1-node1")

    # run grpc-only mode directly with existing chain state
    with (okami_cluster.base_dir / "node1.log").open("a") as logfile:
        proc = subprocess.Popen(
            [
                okami_cluster.chain_binary,
                "start",
                "--grpc-only",
                "--home",
                okami_cluster.base_dir / "node1",
            ],
            stdout=logfile,
        ...
        try:
            ...
            grpc_port = ports.grpc_port(okami_cluster.base_port(1))
            ...
            addr = okami_cluster.cosmos_cli(0).consensus_address()
