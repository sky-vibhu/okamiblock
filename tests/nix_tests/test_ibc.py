...
def ibc(request, tmp_path_factory):
    """
    prepare IBC network with an okami chain
...
def test_ibc_transfer_with_hermes(ibc):
    ...
    dst_addr = ibc.chains["okami"].cosmos_cli().address("signer2")
    old_dst_balance = get_balance(ibc.chains["okami"], dst_addr, dst_denom)
    new_dst_balance = 0

    def check_balance_change():
        nonlocal new_dst_balance
        new_dst_balance = get_balance(ibc.chains["okami"], dst_addr, dst_denom)
    ...
    fee = int(events["tx"]["fee"].removesuffix("aOKM"))
...
def test_okami_ibc_transfer(ibc):
    """
    test sending aOKM from okami to crypto-org-chain using cli.
    """
    assert_ready(ibc)
...
def test_okami_ibc_transfer_acknowledgement_error(ibc):
    """
    test sending aOKM from okami to crypto-org-chain using cli
