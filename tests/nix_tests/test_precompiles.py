...
def ibc(request, tmp_path_factory):
    ...
    network = prepare_network(path, name, [okami_build, "chainmain"])
    yield from network
...
def test_ibc_transfer(ibc):
    ...
    old_dst_balance = get_balance(ibc.chains["chainmain"], dst_addr, OKAMI_IBC_DENOM)

    pc = get_precompile_contract(ibc.chains["okami"].w3, "ICS20I")
    ...
    def check_balance_change():
        ...
        new_dst_balance = get_balance(
            ibc.chains["chainmain"], dst_addr, OKAMI_IBC_DENOM
        )
...
def test_staking(ibc):
    ...
    tx = pc.functions.delegate(ADDRS["signer2"], validator_addr, amt).build_transaction(
        {"from": ADDRS["signer2"], "gasPrice": okami_gas_price}
    )
    ...
    new_src_balance = get_balance(okami, del_addr, src_denom)
    assert old_src_balance - amt - fee == new_src_balance
...
def test_staking_via_sc(ibc):
    ...
    approve_tx = contract.functions.testApprove(
        ...
        {"from": ADDRS["signer1"], "gasPrice": okami_gas_price, "gas": 60000}
    )

    gas_estimation = okami.w3.eth.estimate_gas(approve_tx)
    ...
    delegate_tx = contract.functions.testDelegate(
        ...
        {"from": ADDRS["signer1"], "gasPrice": okami_gas_price, "gas": 180000}
    )
    gas_estimation = okami.w3.eth.estimate_gas(delegate_tx)
