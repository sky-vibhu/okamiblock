...
from .network import build_patched_okamid, setup_custom_okami
from .utils import (
    supervisorctl,
    update_okami_bin,
    update_node_cmd,
...
def custom_okami(tmp_path_factory):
    ...
    broken_binary = build_patched_okamid("broken-okamid")
    print(broken_binary)
    ...
    yield from setup_custom_okami(
        ...
        post_init=update_okami_bin(broken_binary, [1]),
        wait_port=False,
    )


def test_rollback(custom_okami):
    """
    ...
    print("stop node1")
    supervisorctl(custom_okami.base_dir / "../tasks.ini", "stop", "okami_9000-1-node1")

    print("do rollback on node1")
    cli1.rollback()

    print("switch to normal binary")
    update_node_cmd(custom_okami.base_dir, "okamid", 1)
    supervisorctl(custom_okami.base_dir / "../tasks.ini", "update")
    wait_for_port(target_port)
