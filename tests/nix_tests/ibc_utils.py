...
from .utils import (
    ...
    setup_stride,
    update_okami_bin,
    update_okamid_and_setup_stride,
    wait_for_port,
)

# aOKM IBC representation on another chain connected via channel-0.
OKAMI_IBC_DENOM = "ibc/8EAC8061F4499F03D2D1419A3E73D346289AE9DB89CAB1486B72539572B1915E"
# uosmo IBC representation on the Okami chain.
...
EVM_CHAINS = ["okami_9000", "chainmain-1"]
...
def get_okami_generator(
    ...
    """
    setup okami with custom config
    depending on the build
    """
    post_init_func = None
    if is_rocksdb:
        file = memiavl_config(tmp_path, file)
        gen = setup_custom_okami(
            tmp_path,
            26710,
        ...
        if custom_scenario:
            ...
            modified_bin = build_patched_okamid(custom_scenario)
            post_init_func = update_okami_bin(modified_bin)
            if stride_included:
                post_init_func = update_okamid_and_setup_stride(modified_bin)
            gen = setup_custom_okami(
                tmp_path,
                26700,
                Path(__file__).parent / file,
...
def prepare_network(
    ...
    for chain in chain_names:
        ...
        # okami is the first chain
        # set it up and the relayer
        if "okami" in chain_name:
            # setup okami with the custom config
            # depending on the build
            gen = get_okami_generator(
                tmp_path,
                file,
                "-rocksdb" in chain,
            ...
            okami = next(gen)
            # wait for grpc ready
            wait_for_port(ports.grpc_port(okami.base_port(0)))  # okami grpc
            # setup relayer
    ...
    okami.supervisorctl("start", "relayer-demo")
...
def hermes_transfer(ibc, other_chain_name="chainmain-1", other_chain_denom="basecro"):
    assert_ready(ibc)
    # chainmain-1 -> okami_9000-1
    my_ibc0 = other_chain_name
    my_ibc1 = "okami_9000-1"
