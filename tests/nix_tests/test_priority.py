...
def custom_okami(tmp_path_factory):
    ...
    yield from setup_okami(path, 26800, long_timeout_commit=True)
...
def custom_okami_rocksdb(tmp_path_factory):
    ...
    # run with long timeout commit to ensure all
    # txs are included in the same block
    yield from setup_okami_rocksdb(path, 26810, long_timeout_commit=True)
...
def test_priority(okami_cluster):
    """
...
def test_native_tx_priority(okami_cluster):
    ...
    test_cases = [
        {
            ...
            "amount": "1000aOKM",
            "gas_prices": f"{base_fee + PRIORITY_REDUCTION * 600000}aOKM",
        ...
        },
        {
            ...
            "amount": "1000aOKM",
            "gas_prices": f"{base_fee + PRIORITY_REDUCTION * 400000}aOKM",
    ...
    for tc in test_cases:
        ...
        gas_price = int(tc["gas_prices"].removesuffix("aOKM"))
