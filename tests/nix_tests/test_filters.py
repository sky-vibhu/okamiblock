...
from .network import setup_custom_okami, setup_okami
from .utils import (
    ADDRS,
    CONTRACTS,
    deploy_contract,
...
def custom_okami(tmp_path_factory):
    ...
    yield from setup_okami(path, 26200)


@pytest.fixture(scope="module")
def okami_indexer(tmp_path_factory):
...
@pytest.fixture(
    scope="module",
    params=["okami", "geth", "okami-ws", "enable-indexer", "okami-rocksdb"],
)
def cluster(request, custom_okami, okami_indexer, okami_rocksdb, geth):
    """
    run on both okami and geth
    """
