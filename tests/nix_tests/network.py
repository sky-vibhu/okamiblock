...
def setup_okami(path, base_port, long_timeout_commit=False):
    ...
    yield from setup_custom_okami(path, base_port, cfg)
...
def create_snapshots_dir(path, base_port, config, n_nodes=2):
    for idx in range(n_nodes):
        data_snapshots_dir = path / "okami_9000-1" / f"node{idx}" / "data" / "snapshots"
        os.makedirs(data_snapshots_dir, exist_ok=True)


def setup_okami_rocksdb(path, base_port, long_timeout_commit=False):
    """
    setup_okami_rocksdb returns an Okami chain compiled with RocksDB
...
def setup_custom_okami(
    ...
    try:
        ...
        yield Okami(
            path / "okami_9000-1", chain_binary=chain_binary or DEFAULT_CHAIN_BINARY
        )
    finally:
        os.killpg(os.getpgid(proc.pid), signal.SIGTERM)
        proc.wait()
...
def build_patched_okamid(patch_nix_file):
    """
    build the binary modified for a custom scenario
    e.g. allow to register WOKAMI token
