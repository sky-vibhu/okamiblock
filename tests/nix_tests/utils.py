...
OKAMI_ADDRESS_PREFIX = "okami"
DEFAULT_DENOM = "aOKM"
WOKAMI_ADDRESS = Web3.toChecksumAddress("0xcc491f589b45d4a3c679016195b3fb87d7848210")
TEST_CONTRACTS = {
    ...
    "ICS20I": "okami/ics20/ICS20I.sol",
    "DistributionI": "okami/distribution/DistributionI.sol",
    "StakingI": "okami/staking/StakingI.sol",
    "StakingCaller": "okami/staking/testdata/StakingCaller.sol",
    "IStrideOutpost": "okami/outposts/stride/IStrideOutpost.sol",
    "IOsmosisOutpost": "okami/outposts/osmosis/IOsmosisOutpost.sol",
    "IERC20": "okami/erc20/IERC20.sol",
}

OSMOSIS_POOLS = {
    "Okami_Osmo": Path(__file__).parent / "osmosis/okamiOsmosisPool.json",
}
...
def eth_to_bech32(addr, prefix=OKAMI_ADDRESS_PREFIX):
...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:okami_9000-1-node{i}":
            ini[section].update(
                {
...
def update_okamid_and_setup_stride(modified_bin):
    def inner(path, base_port, config):
        update_okami_bin(modified_bin)(path, base_port, config)
        setup_stride()(path, base_port, config)

    return inner


def update_okami_bin(modified_bin, nodes=[0, 1]):
    """
    updates the okami binary with a patched binary.
