...
from .network import CosmosChain, Okami
from .utils import (
    ADDRS,
    KEYS,
    OSMOSIS_POOLS,
...
def ibc(request, tmp_path_factory):
    ...
    network = prepare_network(path, name, [okami_build, "osmosis"])
    yield from network


def test_osmosis_swap(ibc):
    assert_ready(ibc)
    okami: Okami = ibc.chains["okami"]
    osmosis: CosmosChain = ibc.chains["osmosis"]

    okami_addr = ADDRS["signer2"]
    ...
    # --------- Transfer Osmo to Okami
    transfer_osmo_to_okami(ibc, osmosis_addr, okami_addr)

    # --------- Register Osmosis ERC20 token
    osmo_erc20_addr = register_osmo_token(okami)
    ...
    okami_gas_price = w3.eth.gas_price
    swap_params = {
        "channelID": "channel-0",
        "xcsContract": xcs_contract,
        "sender": okami_addr,
        "input": WOKAMI_ADDRESS,
        "output": osmo_erc20_addr,
        "amount": amt,
        "slippagePercentage": testSlippagePercentage,
        "windowSeconds": testWindowSeconds,
        "swapReceiver": eth_to_bech32(okami_addr),
    }
    tx = pc.functions.swap(swap_params).build_transaction(
        {"from": okami_addr, "gasPrice": okami_gas_price, "gas": 30000000}
    )
    gas_estimation = okami.w3.eth.estimate_gas(tx)
    ...
    def check_erc20_balance_change():
        ...
        new_erc20_balance = erc20_balance(w3, osmo_erc20_addr, okami_addr)
...
def setup_osmos_chains(ibc):
    ...
    '''
    # Send Okami to Osmosis to be able to set up pools
    send_okami_to_osmos(ibc)

    osmosis = ibc.chains["osmosis"]
    osmosis_cli = osmosis.cosmos_cli()
    osmosis_addr = osmosis_cli.address("signer2")

    # create okami <> osmo pool
    pool_id = create_osmosis_pool(
        osmosis_cli, osmosis_addr, OSMOSIS_POOLS["Okami_Osmo"]
    )
    ...
    contracts_to_store = {
        ...
        "CrosschainSwap": {
            "get_instantiate_params": lambda x, y, z: f'{{"governor":"{x}", "swap_contract": "{y}", "channels": [["okami","{z}"]]}}',  # noqa: 501 - ignore line length lint
        },
    }
    ...
    # in the router one execute function `set_route` to have a route for okami within the swap router contract
    # set input 'aOKM', output 'uosmo' route
    set_swap_route(
        osmosis_cli, osmosis_addr, swap_contract_addr, pool_id, OKAMI_IBC_DENOM, "uosmo"
    )

    return xcs_addr


def send_okami_to_osmos(ibc):
    src_chain = ibc.chains["okami"]
    ...
    old_dst_balance = get_balance(dst_chain, dst_addr, OKAMI_IBC_DENOM)

    pc = get_precompile_contract(src_chain.w3, "ICS20I")
    okami_gas_price = src_chain.w3.eth.gas_price
    ...
    tx_hash = pc.functions.transfer(
        ...
        0,
        "",
    ).transact({"from": ADDRS["signer2"], "gasPrice": okami_gas_price})
    ...
    def check_balance_change():
        ...
        new_dst_balance = get_balance(dst_chain, dst_addr, OKAMI_IBC_DENOM)
...
def transfer_osmo_to_okami(ibc, src_addr, dst_addr):
    src_chain: CosmosChain = ibc.chains["osmosis"]
    dst_chain: Okami = ibc.chains["okami"]
...
def register_osmo_token(okami):
    ...
    proposal = {
        ...
        "description": "The IBC representation of OSMO on Okami chain",
        "metadata": [ERC_OSMO_META],
        "deposit": "1aOKM",
    }
    proposal_id = register_ibc_coin(okami_cli, proposal)
    assert (
        int(proposal_id) > 0
    ...
    approve_proposal(okami, proposal_id)
    # query token pairs and get contract address
    pairs = okami_cli.get_token_pairs()
