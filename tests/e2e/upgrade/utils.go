...
// OkamiVersions is a custom comparator for sorting semver version strings.
type OkamiVersions []string

// Len is the number of stored versions..
func (v OkamiVersions) Len() int { return len(v) }

// Swap swaps the elements with indexes i and j. It is needed to sort the slice.
func (v OkamiVersions) Swap(i, j int) { v[i], v[j] = v[j], v[i] }

// Less compares semver versions strings properly
func (v OkamiVersions) Less(i, j int) bool {
...
func CheckLegacyProposal(version string) bool {
	...
	cmp := OkamiVersions([]string{version, "v10.0.0"})
...
func RetrieveUpgradesList(upgradesPath string) ([]string, error) {
	...
	sort.Sort(OkamiVersions(versions))

	return versions, nil
}

// ExportState executes the  'docker cp' command to copy container .okamid dir
...
func (m *Manager) ExportState(targetDir string) error {
	...
	cmd := exec.Command(
		"docker",
		"cp",
		fmt.Sprintf("%s:/root/.okamid", m.ContainerID()),
		targetDir,
	)
	return cmd.Run()
}
