...
// runInitialNode builds a docker image capable of running an Okami node with the given version.
...
func (s *IntegrationTestSuite) runInitialNode(version upgrade.VersionConfig) {
	...
	s.Require().NoError(err, "can't build container with Okami version: %s", version.ImageTag)
	...
	s.Require().NoError(err, "can't run node with Okami version: %s", version)
...
// runNodeWithCurrentChanges builds a docker image using the current branch of the Okami repository.
...
func (s *IntegrationTestSuite) runNodeWithCurrentChanges() {
	...
	err = s.upgradeManager.RunNode(node)
	s.Require().NoError(err, "can't run node Okami using branch %s", branch)
}
...
func (s *IntegrationTestSuite) proposeUpgrade(name, target string) {
	...
	// if Okami is lower than v10.x.x no need to use the legacy proposal
	currentVersion, err := s.upgradeManager.GetNodeVersion(ctx)
	s.Require().NoError(err, "can't get current Okami version")
	...
	exec, err := s.upgradeManager.CreateSubmitProposalExec(
		...
		isLegacyProposal,
		"--fees=10000000000000000aOKM",
		"--gas=500000",
	)
	s.Require().NoErrorf(
		err,
		"can't create the proposal to upgrade Okami to %s at height %d with name %s",
	...
	s.Require().NoErrorf(
		err,
		"failed to submit proposal to upgrade Okami to %s at height %d\nstdout: %s,\nstderr: %s",
...
func (s *IntegrationTestSuite) voteForProposal(id int) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	exec, err := s.upgradeManager.CreateVoteProposalExec(s.upgradeParams.ChainID, id, "--fees=10000000000000000aOKM", "--gas=500000")
...
func (s *IntegrationTestSuite) upgrade(targetRepo, targetVersion string) {
	...
	// export node .okamid to local build/
	...
	node.SetCmd([]string{"okamid", "start", fmt.Sprintf("--chain-id=%s", s.upgradeParams.ChainID), fmt.Sprintf("--home=%s.okamid", rootDir)})
	err = s.upgradeManager.RunNode(node)
