...
function checkTestEnv () {
  const argv = yargs(hideBin(process.argv))
    ...
    .example('$0 --network okami', 'run all tests using okami network')
    .example(
      '$0 --network okami --allowTests=test1,test2',
      'run only test1 and test2 using okami network'
    )
    .help('h')
    .alias('h', 'help')
    .describe('network', 'set which network to use: ganache|okami')
    .describe(
      'batch',
    ...
    .boolean('verbose-log')
    .describe('verbose-log', 'print okamid output, default false').argv
  ...
  if (!argv.network) {
    ...
    if (argv.network !== 'okami' && argv.network !== 'ganache') {
      panic('network is invalid. Must be ganache or okami')
    } else {
...
function loadTests (runConfig) {
  ...
  fs.readdirSync(path.join(__dirname, 'suites')).forEach((dirname) => {
    ...
    try {
      ...
      const needScripts = ['test-ganache', 'test-okami']
...
function performTestSuite ({ testName, network }) {
  const cmd = network === 'ganache' ? 'test-ganache' : 'test-okami'
...
function setupNetwork ({ runConfig, timeout }) {
  if (runConfig.network !== 'okami') {
    // no need to start ganache. Truffle will start it
    return
  }

  // Spawn the okami process

  const spawnPromise = new Promise((resolve, reject) => {
    const serverStartedLog = 'Starting JSON-RPC server'
    const serverStartedMsg = 'okamid started'

    const okamidProc = spawn('../e2e/init-node.sh', {
      cwd: __dirname,
      stdio: ['ignore', 'pipe', 'pipe']
    })

    logger.info(`Starting okamid process... timeout: ${timeout}ms`)
    if (runConfig.verboseLog) {
      okamidProc.stdout.pipe(process.stdout)
    }

    okamidProc.stdout.on('data', (d) => {
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start okamid timeout!')), timeout)
  })
