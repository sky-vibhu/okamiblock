...
func (suite *KeeperTestSuite) TestGetCirculatingSupplyAndInflationRate() {
	...
	bondedCoins := sdk.NewDecCoin(okamitypes.AttoOkami, bondedAmt)
	...
	testCases := []struct {
	...
	}{
		{
			...
			sdk.TokensFromConsensusPower(400_000_000, okamitypes.PowerReduction).Sub(bondedAmt),
			func() {
		...
		{
			"high supply",
			sdk.TokensFromConsensusPower(800_000_000, okamitypes.PowerReduction).Sub(bondedAmt),
			func() {},
		...
		},
		{
			...
			sdk.TokensFromConsensusPower(200_000_000, okamitypes.PowerReduction).Sub(bondedAmt),
			func() {},
			math.LegacyZeroDec(),
		},
	}
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			suite.ctx = suite.ctx.WithChainID("okami_9001-1")
			tc.malleate()
			...
			teamAlloc := sdk.NewDecCoin(
				types.DefaultInflationDenom,
				sdk.TokensFromConsensusPower(int64(200_000_000), okamitypes.PowerReduction),
			)
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.isMainnet {
				...
				suite.ctx = suite.ctx.WithChainID("okami_9999-666")
			}
			tc.malleate()
