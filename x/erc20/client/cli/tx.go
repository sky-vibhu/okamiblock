...
func NewConvertCoinCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) == 2 {
				receiver = args[1]
				if err := okamitypes.ValidateAddress(receiver); err != nil {
...
func NewConvertERC20Cmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			contract := args[0]
			if err := okamitypes.ValidateAddress(contract); err != nil {
...

{
  "metadata": [
	...
	cmd.Flags().String(cli.FlagDeposit, "1aOKM", "deposit of proposal")
