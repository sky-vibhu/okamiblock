		...
		Context("with existing coins", func() {
			...
			Describe("for a single Cosmos Coin", func() {
				BeforeEach(func() {
					...
					_, err = testutil.Delegate(s.ctx, s.app, privKey, sdk.NewCoin("aOKM", math.NewInt(500000000000000000)), s.validator)
					s.Require().NoError(err)
...
func submitRegisterCoinProposal(ctx sdk.Context, appOkami *app.Okami, pk *ethsecp256k1.PrivKey, metadata []banktypes.Metadata) (id uint64, err error) {
	content := types.NewRegisterCoinProposal("test Coin", "foo", metadata...)
	return testutil.SubmitProposal(ctx, appOkami, pk, content, 8)
}

func submitRegisterERC20Proposal(ctx sdk.Context, appOkami *app.Okami, pk *ethsecp256k1.PrivKey, addrs []string) (id uint64, err error) {
	content := types.NewRegisterERC20Proposal("test token", "foo", addrs...)
	return testutil.SubmitProposal(ctx, appOkami, pk, content, 8)
}

func convertCoin(ctx sdk.Context, appOkami *app.Okami, pk *ethsecp256k1.PrivKey, coin sdk.Coin) {
	...
	res, err := testutil.DeliverTx(ctx, appOkami, pk, nil, convertCoinMsg)
	s.Require().NoError(err)

	Expect(res.IsOK()).To(BeTrue(), "failed to convert coin: %s", res.Log)
}

func convertERC20(ctx sdk.Context, appOkami *app.Okami, pk *ethsecp256k1.PrivKey, amt math.Int, contract common.Address) {
	addrBz := pk.PubKey().Address().Bytes()

	convertERC20Msg := types.NewMsgConvertERC20(amt, sdk.AccAddress(addrBz), contract, common.BytesToAddress(addrBz))
	res, err := testutil.DeliverTx(ctx, appOkami, pk, nil, convertERC20Msg)
	s.Require().NoError(err)
