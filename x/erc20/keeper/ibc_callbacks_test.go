...
import (
	...
	"github.com/okamiblock/okamiblock/x/erc20/keeper"
...
func (suite *KeeperTestSuite) TestOnRecvPacket() {
	...
	ethsecpAddrOkami := sdk.AccAddress(ethPk.PubKey().Address()).String()
	ethsecpAddrCosmos := sdk.MustBech32ifyAddressBytes(sdk.Bech32MainPrefix, ethsecpAddr)

	// Setup Cosmos <=> Okami IBC relayer
	sourceChannel := "channel-292"
	okamiChannel := "channel-3"
	path := fmt.Sprintf("%s/%s", transfertypes.PortID, okamiChannel)
	...
	testCases := []struct {
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(registeredDenom, "100", ethsecpAddrOkami, ethsecpAddrCosmos, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, okamiChannel, timeoutHeight, 0)
			},
			receiver:      secpAddr,
			disableERC20:  true,
			ackSuccess:    true,
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(registeredDenom, "100", "okami", ethsecpAddrCosmos, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, okamiChannel, timeoutHeight, 0)
			},
			receiver:      secpAddr,
			ackSuccess:    false,
			checkBalances: false,
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(registeredDenom, "100", ethsecpAddrOkami, "badbadhf0468jjpe6m6vx38s97z2qqe8ldu0njdyf625", "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, okamiChannel, timeoutHeight, 0)
			},
			receiver:      secpAddr,
		...
		},
		{
			...
			malleate: func() {
				...
				transfer := transfertypes.NewFungibleTokenPacketData(prefixedDenom, "100", secpAddrCosmos, ethsecpAddrOkami, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, okamiChannel, timeoutHeight, 0)
			},
			ackSuccess:    true,
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(erc20Denom, "100", secpAddrCosmos, ethsecpAddrOkami, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, okamiChannel, timeoutHeight, 0)
			},
			ackSuccess:    true,
		...
		},
		{
			...
			malleate: func() {
				...
				otherSecpAddrOkami := sdk.AccAddress(pk1.PubKey().Address()).String()
				transfer := transfertypes.NewFungibleTokenPacketData(prefixedDenom, "500", otherSecpAddrOkami, ethsecpAddrOkami, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, okamiChannel, timeoutHeight, 0)
			},
			ackSuccess: true,
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(registeredDenom, "100", secpAddrCosmos, secpAddrOkami, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, okamiChannel, timeoutHeight, 0)
			},
			receiver:      secpAddr,
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData("b/d//s/ss/", "100", ethsecpAddrOkami, ethsecpAddrCosmos, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, okamiChannel, timeoutHeight, 0)
			},
			receiver:      secpAddr,
		...
		},
		{
			...
			malleate: func() {
				...
				transfer := transfertypes.NewFungibleTokenPacketData(prefixedDenom, "1000", secpAddrCosmos, ethsecpAddrOkami, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, okamiChannel, timeoutHeight, 0)
			},
			receiver:      ethsecpAddr,
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			suite.app.IBCKeeper.ChannelKeeper.SetChannel(suite.ctx, transfertypes.PortID, okamiChannel, channel)

			// Set Next Sequence Send
			suite.app.IBCKeeper.ChannelKeeper.SetNextSequenceSend(suite.ctx, transfertypes.PortID, okamiChannel, 1)
			...
			// Fund receiver account with OKAMI, ERC20 coins and IBC vouchers
...
func (suite *KeeperTestSuite) TestConvertCoinToERC20FromPacket() {
	senderAddr := "okami1x2w87cvt5mqjncav4lxy8yfreynn273xn5335v"
	...
	testCases := []struct {
	...
	}{
		{
			...
			malleate: func() transfertypes.FungibleTokenPacketData {
				return transfertypes.NewFungibleTokenPacketData("aOKM", "10", "", "", "")
			},
			expPass: false,
		},
		{
			...
			malleate: func() transfertypes.FungibleTokenPacketData {
				return transfertypes.NewFungibleTokenPacketData("aOKM", "10", senderAddr, "", "")
			},
			expPass: true,
		},
		{
		...
		},
		{
			...
			malleate: func() transfertypes.FungibleTokenPacketData {
				...
				coinOkami := sdk.NewCoin(pair.Denom, math.NewInt(10))
