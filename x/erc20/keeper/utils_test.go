...
import (
	...
	"github.com/okamiblock/okamiblock/contracts"
	"github.com/okamiblock/okamiblock/crypto/ethsecp256k1"
	ibctesting "github.com/okamiblock/okamiblock/ibc/testing"
	"github.com/okamiblock/okamiblock/server/config"
	"github.com/okamiblock/okamiblock/testutil"
	utiltx "github.com/okamiblock/okamiblock/testutil/tx"
	teststypes "github.com/okamiblock/okamiblock/types/tests"
...
func (suite *KeeperTestSuite) SetupIBCTest() {
	...
	suite.OkamiChain = suite.coordinator.GetChain(ibcgotesting.GetChainID(1))
	suite.IBCOsmosisChain = suite.coordinator.GetChain(ibcgotesting.GetChainID(2))
	suite.IBCCosmosChain = suite.coordinator.GetChain(ibcgotesting.GetChainID(3))
	suite.coordinator.CommitNBlocks(suite.OkamiChain, 2)
	suite.coordinator.CommitNBlocks(suite.IBCOsmosisChain, 2)
	suite.coordinator.CommitNBlocks(suite.IBCCosmosChain, 2)

	s.app = suite.OkamiChain.App.(*app.Okami)
	evmParams := s.app.EvmKeeper.GetParams(s.OkamiChain.GetContext())
	evmParams.EvmDenom = utils.BaseDenom
	err := s.app.EvmKeeper.SetParams(s.OkamiChain.GetContext(), evmParams)
	suite.Require().NoError(err)

	// s.app.FeeMarketKeeper.SetBaseFee(s.OkamiChain.GetContext(), big.NewInt(1))

	// Set block proposer once, so its carried over on the ibc-go-testing suite
	validators := s.app.StakingKeeper.GetValidators(suite.OkamiChain.GetContext(), 2)
	cons, err := validators[0].GetConsAddr()
	suite.Require().NoError(err)
	suite.OkamiChain.CurrentHeader.ProposerAddress = cons.Bytes()

	err = s.app.StakingKeeper.SetValidatorByConsAddr(suite.OkamiChain.GetContext(), validators[0])
	suite.Require().NoError(err)

	_, err = s.app.EvmKeeper.GetCoinbaseAddress(suite.OkamiChain.GetContext(), sdk.ConsAddress(suite.OkamiChain.CurrentHeader.ProposerAddress))
	suite.Require().NoError(err)
	// Mint coins locked on the okami account generated with secp.
	amt, ok := sdkmath.NewIntFromString("1000000000000000000000")
	suite.Require().True(ok)
	coinOkami := sdk.NewCoin(utils.BaseDenom, amt)
	coins := sdk.NewCoins(coinOkami)
	err = s.app.BankKeeper.MintCoins(suite.OkamiChain.GetContext(), inflationtypes.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToAccount(suite.OkamiChain.GetContext(), inflationtypes.ModuleName, suite.OkamiChain.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)
	...
	err = s.app.BankKeeper.MintCoins(s.OkamiChain.GetContext(), types.ModuleName, coins)
	s.Require().NoError(err)
	coins = sdk.NewCoins(sdk.NewCoin(teststypes.UatomIbcdenom, sdkmath.NewInt(100)))
	err = s.app.BankKeeper.MintCoins(s.OkamiChain.GetContext(), types.ModuleName, coins)
	s.Require().NoError(err)

	// Mint coins on the osmosis side which we'll use to unlock our aOKM
	...
	// Mint coins on the cosmos side which we'll use to unlock our aOKM
	...
	err = s.app.Erc20Keeper.SetParams(suite.OkamiChain.GetContext(), params)
	suite.Require().NoError(err)

	suite.pathOsmosisOkami = ibctesting.NewTransferPath(suite.IBCOsmosisChain, suite.OkamiChain) // clientID, connectionID, channelID empty
	suite.pathCosmosOkami = ibctesting.NewTransferPath(suite.IBCCosmosChain, suite.OkamiChain)
	suite.pathOsmosisCosmos = ibctesting.NewTransferPath(suite.IBCCosmosChain, suite.IBCOsmosisChain)
	ibctesting.SetupPath(suite.coordinator, suite.pathOsmosisOkami) // clientID, connectionID, channelID filled
	ibctesting.SetupPath(suite.coordinator, suite.pathCosmosOkami)
	ibctesting.SetupPath(suite.coordinator, suite.pathOsmosisCosmos)
	suite.Require().Equal("07-tendermint-0", suite.pathOsmosisOkami.EndpointA.ClientID)
	suite.Require().Equal("connection-0", suite.pathOsmosisOkami.EndpointA.ConnectionID)
	suite.Require().Equal("channel-0", suite.pathOsmosisOkami.EndpointA.ChannelID)

	coinOkami = sdk.NewCoin(utils.BaseDenom, sdkmath.NewInt(1000000000000000000))
	coins = sdk.NewCoins(coinOkami)
	err = s.app.BankKeeper.MintCoins(suite.OkamiChain.GetContext(), types.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToModule(suite.OkamiChain.GetContext(), types.ModuleName, authtypes.FeeCollectorName, coins)
	suite.Require().NoError(err)
}
...
// to the Okami chain (used on IBC tests)
func (suite *KeeperTestSuite) DeployContractToChain(name, symbol string, decimals uint8) (common.Address, error) {
	return testutil.DeployContract(
		s.OkamiChain.GetContext(),
		s.OkamiChain.App.(*app.Okami),
		suite.OkamiChain.SenderPrivKey,
		suite.queryClientEvm,
		contracts.ERC20MinterBurnerDecimalsContract,
		name, symbol, decimals,
	)
...
// Send back coins (from path endpoint B to A). In case of IBC coins need to provide ibcCoinMetadata (<port>/<channel>/<denom>, e.g.: "transfer/channel-0/aOKM") as input parameter.
