...
// AddEVMExtensions implements types.EVMKeeper.
func (k *Keeper) AddEVMExtensions(ctx sdk.Context, precompiles ...vm.PrecompiledContract) error {
	panic("unimplemented")
}

// IsAvailablePrecompile implements types.EVMKeeper.
func (k *Keeper) IsAvailablePrecompile(addr common.Address) bool {
	panic("unimplemented")
}
...
func (k *Keeper) WithChainID(ctx sdk.Context) {
	chainID, err := okamitypes.ParseChainID(ctx.ChainID())
	if err != nil {
		panic(err)
	}
	...
	if !(chainID.Cmp(big.NewInt(9001)) == 0 || chainID.Cmp(big.NewInt(9000)) == 0) {
		panic("EVM only supports Okami chain identifiers (9000 or 9001)")
	}
...
func (k *Keeper) GetAccountWithoutBalance(ctx sdk.Context, addr common.Address) *statedb.Account {
	...
	ethAcct, ok := acct.(okamitypes.EthAccountI)
	if ok {
