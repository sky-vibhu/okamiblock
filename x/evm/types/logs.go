...
func (tx TransactionLogs) Validate() error {
	if okamitypes.IsEmptyHash(tx.Hash) {
...
func (log *Log) Validate() error {
	if err := okamitypes.ValidateAddress(log.Address); err != nil {
		return fmt.Errorf("invalid log address %w", err)
	}
	if okamitypes.IsEmptyHash(log.BlockHash) {
		return fmt.Errorf("block hash cannot be the empty %s", log.BlockHash)
	}
	if log.BlockNumber == 0 {
		return errors.New("block number cannot be zero")
	}
	if okamitypes.IsEmptyHash(log.TxHash) {
