...
import (
	...
	okami "github.com/okamiblock/okamiblock/types"
...
func LegacyEIP712Cmd() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(`$ %s debug legacy-eip712 tx.json --chain-id okamid_9000-1`, version.AppName),
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			chainID, err := okami.ParseChainID(clientCtx.ChainID)
			if err != nil {
