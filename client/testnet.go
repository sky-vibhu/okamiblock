...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		coins := sdk.Coins{
			sdk.NewCoin(okamitypes.AttoOkami, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(okamitypes.AttoOkami, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(okamitypes.AttoOkami)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, okamitypes.AttoOkami, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
