...
import (
	...
	okamiapp "github.com/okamiblock/okamiblock/app"
	okamicontracts "github.com/okamiblock/okamiblock/contracts"
	okamiibc "github.com/okamiblock/okamiblock/ibc/testing"
	"github.com/okamiblock/okamiblock/precompiles/authorization"
	cmn "github.com/okamiblock/okamiblock/precompiles/common"

	//"github.com/okamiblock/okamiblock/precompiles/erc20"
	"github.com/okamiblock/okamiblock/precompiles/ics20"
	"github.com/okamiblock/okamiblock/precompiles/testutil"
	"github.com/okamiblock/okamiblock/precompiles/testutil/contracts"
	okamiutil "github.com/okamiblock/okamiblock/testutil"
	okamiutiltx "github.com/okamiblock/okamiblock/testutil/tx"
...
// SetupWithGenesisValSet initializes a new OkamiApp with a validator set and genesis accounts
...
func (s *PrecompileTestSuite) SetupWithGenesisValSet(valSet *tmtypes.ValidatorSet, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) {
	appI, genesisState := okamiapp.SetupTestingApp(cmn.DefaultChainID)()
	app, ok := appI.(*okamiapp.Okami)
	s.Require().True(ok)
	...
	bondAmt := sdk.TokensFromConsensusPower(1, okamitypes.PowerReduction)
	...
	app.InitChain(
		abci.RequestInitChain{
			...
			ConsensusParams: okamiapp.DefaultConsensusParams,
...
func (s *PrecompileTestSuite) NewTestChainWithValSet(coord *ibctesting.Coordinator, valSet *tmtypes.ValidatorSet, signers map[string]tmtypes.PrivValidator) *ibctesting.TestChain {
	...
	addr, priv := okamiutiltx.NewAddrKey()
	s.privKey = priv
	s.address = addr
	// differentAddr is an address generated for testing purposes that e.g. raises the different origin error
	s.differentAddr = okamiutiltx.GenerateAddress()
	s.signer = okamiutiltx.NewSigner(priv)

	baseAcc := authtypes.NewBaseAccount(priv.PubKey().Address().Bytes(), priv.PubKey(), 0, 0)

	acc := &okamitypes.EthAccount{
		BaseAccount: baseAcc,
		CodeHash:    common.BytesToHash(evmtypes.EmptyCodeHash).Hex(),
	}

	amount := sdk.TokensFromConsensusPower(5, okamitypes.PowerReduction)
...
func (s *PrecompileTestSuite) NewTransferAuthorizationWithAllocations(ctx sdk.Context, app *okamiapp.Okami, grantee, granter common.Address, allocations []transfertypes.Allocation) error {
...
func (s *PrecompileTestSuite) NewTransferAuthorization(ctx sdk.Context, app *okamiapp.Okami, grantee, granter common.Address, path *ibctesting.Path, coins sdk.Coins, allowList []string) error {
...
func (s *PrecompileTestSuite) setupIBCTest() {
	...
	s.app = s.chainA.App.(*okamiapp.Okami)
	...
	s.transferPath = okamiibc.NewTransferPath(s.chainA, s.chainB) // clientID, connectionID, channelID empty
	okamiibc.SetupPath(s.coordinator, s.transferPath)             // clientID, connectionID, channelID filled
...
// TODO upstream this change to okami (adding gasPrice)
// DeployContract deploys a contract with the provided private key,
// compiled contract data and constructor arguments
func DeployContract(
	ctx sdk.Context,
	okamiapp *okamiapp.Okami,
	...
	chainID := okamiapp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	nonce := okamiapp.EvmKeeper.GetNonce(ctx, from)
	...
	gas, err := okamiutiltx.GasLimit(ctx, from, data, queryClientEvm)
	if err != nil {
		return common.Address{}, err
	}
	...
	msgEthereumTx := evmtypes.NewTx(&evmtypes.EvmTxArgs{
		...
		Nonce:     nonce,
		GasLimit:  gas,
		GasFeeCap: okamiapp.FeeMarketKeeper.GetBaseFee(ctx),
	...
	res, err := okamiutil.DeliverEthTx(okamiapp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := okamiutil.CheckEthTxResponse(res, okamiapp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func (s *PrecompileTestSuite) DeployERC20Contract(chain *ibctesting.TestChain, name, symbol string, decimals uint8) (common.Address, error) {
	addr, err := DeployContract(
		...
		gasPrice,
		s.queryClientEVM,
		okamicontracts.ERC20MinterBurnerDecimalsContract,
		name,
		symbol,
		decimals,
	)
...
func (s *PrecompileTestSuite) setupERC20ContractTests(amount *big.Int) common.Address {
	...
	defaultERC20CallArgs := contracts.CallArgs{
		...
		ContractABI:  okamicontracts.ERC20MinterBurnerDecimalsContract.ABI,
		PrivKey:      s.privKey,
		GasPrice:     gasPrice,
	}
	...
	mintCheck := testutil.LogCheckArgs{
		ABIEvents: okamicontracts.ERC20MinterBurnerDecimalsContract.ABI.Events,
	...
	balance := s.app.Erc20Keeper.BalanceOf(
		s.chainA.GetContext(),
		okamicontracts.ERC20MinterBurnerDecimalsContract.ABI,
		erc20Addr,
		s.address,
	)
