...
func (s *PrecompileTestSuite) TestIncreaseAllowance() {
	...
	testCases := []allowanceTestCase{
		...
		{
			"pass - increase allowance by 1 OKAMI for a single allocation with a single coin denomination",
			func() []interface{} {
				path := NewTransferPath(s.chainA, s.chainB)
				s.coordinator.Setup(path)
				err := s.NewTransferAuthorization(s.ctx, s.app, differentAddress, s.address, path, defaultCoins, nil)
		...
		{
			"pass - increase allowance by 1 Okami for multiple allocations with a single coin denomination",
			func() []interface{} {
				path := NewTransferPath(s.chainA, s.chainB)
				s.coordinator.Setup(path)
...
func (s *PrecompileTestSuite) TestDecreaseAllowance() {
	...
	testCases := []allowanceTestCase{
		...
		{
			"pass - decrease allowance by 1 OKAMI for a single allocation with a single coin denomination",
			func() []interface{} {
				path := NewTransferPath(s.chainA, s.chainB)
				s.coordinator.Setup(path)
				err := s.NewTransferAuthorization(s.ctx, s.app, differentAddress, s.address, path, defaultCoins, nil)
		...
		{
			"pass - decrease allowance by 0.5 Okami for multiple allocations with a single coin denomination",
			func() []interface{} {
				path := NewTransferPath(s.chainA, s.chainB)
				s.coordinator.Setup(path)
				allocations := []transfertypes.Allocation{
