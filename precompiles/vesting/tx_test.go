...
var (
	...
	toAddr           = okamiutiltx.GenerateAddress()
	funderAddr       = okamiutiltx.GenerateAddress()
	diffFunderAddr   = okamiutiltx.GenerateAddress()
...
func (s *PrecompileTestSuite) TestCreateClawbackVestingAccount() {
	...
	testCases := []struct {
		...
		},
		{
			...
			malleate: func() []interface{} {
				differentAddr := okamiutiltx.GenerateAddress()
				return []interface{}{
					funderAddr,
					differentAddr,
					false,
	...
	testCases := []struct {
		...
		},
		{
			...
			func() []interface{} {
				...
				err = okamiutil.FundAccount(s.ctx, s.app.BankKeeper, toAddr.Bytes(), sdk.NewCoins(sdk.NewCoin(utils.BaseDenom, math.NewInt(100))))
				return []interface{}{
					s.address,
					toAddr,
					uint64(time.Now().Unix()),
	...
	testCases := []struct {
		...
		},
		{
			...
			func(data []byte) {
				...
				_, ok := account.(okamitypes.EthAccountI)
				s.Require().True(ok)
			},
			false,
			"",
