...
import (
	...
	"github.com/okamiblock/okamiblock/precompiles/vesting"
	"github.com/okamiblock/okamiblock/precompiles/vesting/testdata"
...
func (s *PrecompileTestSuite) CreateTestClawbackVestingAccount(funder, vestingAddr common.Address) {
	...
	err = okamiutil.FundAccount(s.ctx, s.app.BankKeeper, vestingAddr.Bytes(), sdk.NewCoins(sdk.NewCoin(utils.BaseDenom, math.NewInt(100))))
	s.Require().NoError(err)
...
func (s *PrecompileTestSuite) DeployContract(contract evmtypes.CompiledContract) (addr common.Address, err error) {
	addr, err = okamiutil.DeployContract(
		s.ctx,
		s.app,
		s.privKey,
		s.queryClientEVM,
...
func (s *PrecompileTestSuite) NextBlock() {
	var err error
	s.ctx, err = okamiutil.CommitAndCreateNewCtx(s.ctx, s.app, time.Second, nil)
