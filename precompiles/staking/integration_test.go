...
import (
	...
	compiledcontracts "github.com/okamiblock/okamiblock/contracts"
	"github.com/okamiblock/okamiblock/precompiles/authorization"
	cmn "github.com/okamiblock/okamiblock/precompiles/common"
	"github.com/okamiblock/okamiblock/precompiles/distribution"
	"github.com/okamiblock/okamiblock/precompiles/staking"
	"github.com/okamiblock/okamiblock/precompiles/staking/testdata"
	...
	testutiltx "github.com/okamiblock/okamiblock/testutil/tx"
)
...
var _ = Describe("Calling staking precompile directly", func() {
	...
	Describe("Execute approve transaction", func() {
		...
		It("should approve the undelegate method with 1 okami", func() {
			s.SetupApproval(
		...
		It("should approve the redelegate method with 2 okami", func() {
			s.SetupApproval(
		...
		It("should approve the cancel unbonding delegation method with 1 okami", func() {
			s.SetupApproval(
	...
	Describe("Execute increase allowance transaction", func() {
		...
		It("Should increase the allowance of the delegate method with 1 okami", func() {
			increaseArgs := defaultCallArgs.
				WithMethodName(authorization.IncreaseAllowanceMethod).
				WithArgs(
	...
	Describe("Execute decrease allowance transaction", func() {
		...
		It("Should decrease the allowance of the delegate method with 1 okami", func() {
			decreaseArgs := defaultDecreaseArgs.WithArgs(
				s.precompile.Address(), big.NewInt(1e18), []string{staking.DelegateMsg},
			)
	...
	Describe("to revoke an approval", func() {
		...
		It("should not revoke the approval if sent by someone else than the granter", func() {
			...
			err := okamiutil.FundAccountWithBaseDenom(s.ctx, s.app.BankKeeper, differentAddr.Bytes(), 1e18)
	...
	Describe("Redelegations queries", func() {
		...
		It("should return an empty array if no redelegation is found for the given source validator", func() {
			...
			// possible (see https://github.com/okami/cosmos-sdk/blob/e773cf768844c87245d0c737cda1893a2819dd89/x/staking/keeper/querier.go#L361-L373):
			//
...
var _ = Describe("Calling staking precompile via Solidity", func() {
	...
	Context("delegating", func() {
		...
		Context("with approval set", func() {
			...
			It("should not delegate when sending from a different address", func() {
				...
				err := okamiutil.FundAccountWithBaseDenom(s.ctx, s.app.BankKeeper, newAddr, 1e18)
	...
	Context("querying delegation", func() {
		...
		It("which does not exist should return an empty delegation", func() {
			...
			Expect(delOut.Balance.Denom).To(Equal("aOKM"), "expected a different delegation balance")
		})
		...
		It("which exists should return the delegation", func() {
			...
			Expect(delOut.Balance).To(Equal(
				cmn.Coin{Denom: "aOKM", Amount: big.NewInt(1e18)}),
...
var _ = Describe("Batching cosmos and eth interactions", func() {
	...
	BeforeEach(func() {
		...
		contractAddr, err = okamiutil.DeployContract(s.ctx, s.app, s.privKey, s.queryClientEVM, testdata.StakingCallerContract)
		Expect(err).To(BeNil(), "error while deploying the StakingCaller contract")

		// Deploy ERC20 contract
		erc20ContractAddr, err = okamiutil.DeployContract(s.ctx, s.app, s.privKey, s.queryClientEVM, erc20Contract,
