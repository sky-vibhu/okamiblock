	...
	testCases := []struct {
		...
		},
		{
			...
			func() sdk.Tx {
				...
				gas := uint64(200000)
				amount := sdk.NewCoins(sdk.NewCoin(evmtypes.DefaultEVMDenom, sdkmath.NewInt(100*int64(gas))))
				txBuilder, err := suite.CreateTestEIP712TxBuilderMsgSend(from, privKey, "okami_9002-1", gas, amount)
				suite.Require().NoError(err)
				return txBuilder.GetTx()
			}, false, false, false,
		},
		...
		},
		{
			...
			func() sdk.Tx {
				...
				gas := uint64(200000)
				amount := sdk.NewCoins(sdk.NewCoin(evmtypes.DefaultEVMDenom, sdkmath.NewInt(100*int64(gas))))
				txBuilder, err := suite.CreateTestEIP712TxBuilderMsgSend(from, privKey, "okami_9001-1", gas, amount)
				suite.Require().NoError(err)
				return txBuilder.GetTx()
			}, false, false, false,
		},
