...
// EthSetup initializes a new OkamiApp. A Nop logger is set in OkamiApp.
func EthSetup(isCheckTx bool, patchGenesis func(*Okami, simapp.GenesisState) simapp.GenesisState) *Okami {
	return EthSetupWithDB(isCheckTx, patchGenesis, dbm.NewMemDB())
}

// EthSetupWithDB initializes a new OkamiApp. A Nop logger is set in OkamiApp.
func EthSetupWithDB(isCheckTx bool, patchGenesis func(*Okami, simapp.GenesisState) simapp.GenesisState, db dbm.DB) *Okami {
	chainID := utils.TestnetChainID + "-1"
	app := NewOkami(log.NewNopLogger(),
		db,
		nil,
		true,
		map[int64]bool{},
