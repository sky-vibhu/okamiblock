...
import (
	...
	"github.com/okamiblock/okamiblock/indexer"
	ethdebug "github.com/okamiblock/okamiblock/rpc/namespaces/ethereum/debug"
...
`,
	...
	cmd.Flags().String(server.FlagMinGasPrices, "", "Minimum gas prices to accept for transactions; Any fee in a tx must meet this minimum (e.g. 20000000000aOKM)") //nolint:lll
	...
	cmd.Flags().Uint64(srvflags.JSONRPCGasCap, config.DefaultGasCap, "Sets a cap on gas that can be used in eth_call/estimateGas unit is aOKM (0=infinite)")       //nolint:lll
	cmd.Flags().Float64(srvflags.JSONRPCTxFeeCap, config.DefaultTxFeeCap, "Sets a cap on transaction fee that can be sent via the RPC APIs (1 = default 1 okami)") //nolint:lll
...
func startInProcess(ctx *server.Context, clientCtx client.Context, opts StartOptions) (err error) {
	...
	var idxer okamitypes.EVMTxIndexer
